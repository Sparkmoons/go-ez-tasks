...
Коллизия - применение одной хэш-фунции к двум разным ключам. В результате будет поулчен один и тот же индекс. Разрешить ее можно с помощью метода открытой адресации и метода цепочек.
...
При плохом распределении хэша может поулчится ситуация, когда в одну ячейку поместили N элементов и получили связанный список длиной N. Поиск по такому списку в худшем случае будет O(n), что неэффективнее чем поиск по массиву.
...
Функция предстваляет блок параметров, которые все вместе выполняют какую-либо определенную задачу.
