...
3. Копирование: Для полной безопасности можно передавать копии данных в каждую горутину. 
...
Указатель представляет собой переменную, которая содержит адрес в памяти другой переменной.Позволяют напрямую манипулировать памятью, что может быть полезно для эффективного управления ресурсами и повышения производительности.

Оператор &: Получение адреса переменной.

Оператор *: Доступ к значению, находящемуся по адресу указателя (разыменование указателя).
...
Эффективность: Позволяют передавать большие структуры данных в функции без копирования, что может значительно улучшить производительность.

Гибкость: Позволяют изменять значение переменной из другой функции, что может быть полезно для реализации различных алгоритмов и структур данных.

Низкоуровневый контроль: Предоставляют возможность работы с памятью напрямую, что может быть полезно в системном программировании и других областях, требующих высокой производительности.
...
Хэш-функция преобразует входные данные в числовое значение. В дальнейшем оно используется как индекс для конкретной ячейки. Заполняется хэш-таблица путем применения хэш-функции к каждому элементу и размещение его в соответствующей ячейке.
