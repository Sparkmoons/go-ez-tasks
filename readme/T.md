...
Go - императивный (как сделать - прямо пошагово) компилируемый язык программирования со статической типизацией. 
...
Компилируемый язык со статической типизацией данных, асинхронный. Императивный (как сделать) ((есть еще декларативный - что сделать))
...
Слайс - срез []string{}, известен только тип, значения и размер неизвестны. Сравнивать через == или != нельзя, можно сравнивать только с nil.

package main
import "fmt"

Массив: var a[размерность] int (или string, может быть любой тип
...
Структура: type Contact struct {name string  phone int} (данные в новой строке каждый)
Функции:
...
Map - это структура, реализующая операции хэширования. 
...
Перебор массива:
for i,v:=range massiv{} - i выводит порядок в масиве, v выводит значение. Чтобы выводить только значение меняем i на _
...
(
Мьютекс (mutex - mutual extension)
mu sync.Mutex
...
Ключевое отличие слайса (среза) и массива:
