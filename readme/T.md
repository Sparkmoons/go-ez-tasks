...
Значение из массивов можно получать по значению
...
3) 2) sl := mass[3:] - слайс содержит все элементы массива mass после 3 (4, 5, 6, 7, 8 - элементы)
...
Слайсы можно копировать через copy. Нужно создать слайс, куда мы будем копировать и перенести в него данные
```go
//добавление
res := make([]int, 0)
res = append(res, 3)
