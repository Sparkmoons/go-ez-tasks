...
GOROOT - каталог для стандартной библиотеки, вклюячая исполняемые файлы и исходный код. 
...
Размер сслайс - это количество элементов, которые он содержит.
...
Некоторые операции по умолчанию вызывают выделенре памяти в куче, поэтому легко обнаруживаются и исправляются. Пара примеров:

- Строковые переменные неизменяемы. Конкатенацию двух строк приводит к новой аллокации и сборке мусора. В качестве альтернативы можно использовать strings.Builder. 
- Срезы, которые растут за пределы своей емкости реаллоцируются. Как решение - можно заранее выделить срез с помощью make. 
- Когда функция создает локальную пересенную и возвращает указатель на эту переменную, переменная должна быть выделена в куче. 
